...
export function EventsProgramming() {
  ...
  return <section className="masterSection eventsSection" id="events">
    <div className="eventsHeading" data-reveal><div><div className="sectionNumber">11 / Events & Programming</div><h2>What’s happening<br />at <em>SPILL?</em></h2></div><div className="eventFilters" aria-label="Filter events by location">{["All", "Saigon", "Tokyo"].map((city) => <button className={filter === city ? "active" : ""} key={city} onClick={() => setFilter(city)} aria-pressed={filter === city}>{city === "All" ? "All locations" : city}</button>)}</div></div>
    <div className="eventList" aria-live="polite">{visible.map((event, index) => <article key={`${event.city}-${event.title}`}><span>0{index + 1}</span><div><small>{event.city} / {event.type}</small><h3>{event.title}</h3></div><p>{event.note}</p><b>↗</b></article>)}</div>

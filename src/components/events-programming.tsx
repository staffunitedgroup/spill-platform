...
export function EventsProgramming() {
  ...
  return <section className="masterSection eventsSection" id="events">
    ...
    <div className="eventList" aria-live="polite">{visible.map((event, index) => <article key={`${event.city}-${event.title}`}><span>0{index + 1}</span><div><small>{event.city} / {event.type}</small><h3><BrandedText text={event.title} tone="dark" /></h3></div><p>{event.note}</p><b>↗</b></article>)}</div>

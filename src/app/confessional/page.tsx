  ...
  return <main className="confPage">
    ...
    <section className="confHero">
      ...
      <div className="confHeroContent">
        ...
        <h1>Step in.<br />Sit down.<br /><em>SPILL.</em></h1>
    ...
    <section className="confExperience" id="experience">
      ...
      <div className="confSteps">
        {[["01", "Step in"], ["02", "Sit down"], ["03", "Choose a prompt"], ["04", "Press record"], ["05", "SPILL"]].map(([number, label]) => <article key={label} data-conf-reveal><span>{number}</span><h3>{label}</h3></article>)}
      </div>
    ...
    <section className="confPrivacy">
      ...
      <div className="confConsentGrid">{consentChoices.map(([title, copy], index) => <article key={title} data-conf-reveal><span>0{index + 1}</span><h3>{title}</h3><p>{copy}</p></article>)}</div>
    ...
    <section className="confRevenue">
      ...
      <div className="confPriceTable">{pricing.map(([product, price, detail], index) => <article key={product} data-conf-reveal><span>0{index + 1}</span><h3>{product}</h3><strong>{price}</strong><p>{detail}</p></article>)}</div>
    ...
    <section className="confAccess">
      ...
      <div className="confAccessGrid">
        ...
        <article data-conf-reveal><span>02 / Editorial</span><h3>Be invited by SPILL</h3><p>Creators, musicians, artists, founders, travelers, performers, local personalities, and compelling regulars.</p></article>
    ...
    <section className="confRelocatable">
      <div className="confRelocatableCopy" data-conf-reveal><p className="confKicker">10 / Self-contained + relocatable</p><h2>Power.<br />Internet.<br /><em>SPILL anywhere.</em></h2><p>Every booth operates as one complete unit. Move it across the venue, take it to an event, relocate it between properties, or place it with a partner.</p></div>
    ...
    <section className="confExternal">
      <div className="confExternalHeading" data-conf-reveal><p className="confKicker">11 / Phase two</p><h2>First prove SPILL.<br /><em>Then test the city.</em></h2><p>Only after the internal model works should one external unit test whether the experience can attract demand independently.</p></div>
    ...
    <section className="confCityNetwork">
      <div data-conf-reveal><p className="confKicker">12 / SPILL Confessionals around Saigon</p><h2>Every booth becomes<br /><em>a door back to SPILL.</em></h2></div>
    ...
    <section className="confNetwork">
      ...
      <div className="confCities" data-conf-reveal>{["Saigon", "Bangkok", "Tokyo", "Singapore"].map((city, index) => <div key={city}><span>0{index + 1}</span><h3>SPILL {city}</h3>{index < 3 && <b>→</b>}</div>)}</div>
    </section>
    ...
    <section className="confRoadmap">
      ...
      <div className="confRoadmapGrid">{roadmap.map((step, index) => <article key={step} data-conf-reveal><span>{String(index + 1).padStart(2, "0")}</span><h3>{step}</h3>{index < roadmap.length - 1 && <b>→</b>}</article>)}</div>
    </section>
